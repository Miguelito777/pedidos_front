...
export class AuthInterceptor implements HttpInterceptor {
  ...
  intercept(req: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>>  {
    ...
    if(req.url.startsWith(this.appSettings.restOAUTH)){
      if(req.body != null && req.body.recaptcha != undefined){
        authReq = req.clone({setHeaders: {Authorization: `g-response ${req.body.recaptcha}`}});
        //authReq=req.clone();
      }else{
        ...
        if(tokenOAUTH){
          ...
          if(authReq)
            var dateObj = new Date();
            var fecha256 = new cryptoOJ().sha256("CIT-" + this.appSettings.SistemaOAUTH + "-D" + dateObj.getDate() + "-M" + dateObj.getMonth() + "-Y" + dateObj.getFullYear());
            //console.log("CIT-" + this.appSettings.SistemaOAUTH + "-D" + dateObj.getDate() + "-M" + dateObj.getMonth() + "-Y" + dateObj.getFullYear());
            //console.log(fecha256);
            authReq = req.clone({setHeaders: {Authorization: `Bearer ${fecha256}`}});
